...
class TIS:
    ...
    def select_device(self, index):
        ...
        for device in monitor.get_devices():
            ...
            i += 1
            if struc.get_string("type")=='v4l2':
            	print("{} : Model: {} Serial: {} {} ".format(i - 1, struc.get_string("model"), struc.get_string("serial"), struc.get_string("type")))
            	serials.append("{}-{}".format(struc.get_string("serial"), struc.get_string("type")))
            #if struc.get_string("type")=='v4l2':
                #print('device selected!.'); 
                #break

        if i > 0:
            i = index #int(input("Select : "))
            #if i == 0:
            #    return False

            self.serialnumber = serials[i]
    ...
    def select_format(self):
        ...
        for res in formats[formatindex].res_list:
            ...
            if res.width==1440 and res.height==1080:
        ...
        self.open_device(self.serialnumber, width, height, framerate, SinkFormats.BGRA, False)
        return True
...
class VideoCapture():
    def __init__(self, index):
        self.camera = TIS()
        if not self.camera.select_device(index):
            quit(0)
        try:
    ...
    def read(self):
        if self.camera.snap_image(1):  # Snap an image with one second timeout
